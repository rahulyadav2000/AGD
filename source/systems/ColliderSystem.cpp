...
void ColliderSystem::update(Game* game, Entity* entity, float elapsedTime) {
	...
	std::shared_ptr<ColliderComponent> colliderComponent = entity->getColliderComponent();
	
	if (colliderComponent == nullptr && positionComponent == nullptr)
	{
		throw std::runtime_error("Components unavailable!");
	}
	else
	{
		colliderComponent->getBoundingBox().setTopLeft(positionComponent->getPosition());
		colliderComponent->getBoundingBox().setBottomRight(positionComponent->getPosition() + colliderComponent->getSize());
	}
}

std::vector<EntityType> ColliderSystem::getArchetypesEntities() const
{
